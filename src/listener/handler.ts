        ...
        case DomainProvider.EDNS: {
          switch (body.type) {
            case EventType.DOMAIN_REGISTERED: {
              ...
              const prevUser = await client.hget(
                `edns:${net}:domain:${domain}:info`,
                "owner"
              );
              if (prevUser) {
                // Remove the domain from the previous owner
                batch = batch.srem(
                  `edns:${net}:account:${data.owner}:domains`,
                  domain
                );
                ...
                dels.push(
                  `edns:${net}:domain:${domain}:info`,
                  `edns:${net}:domain:${domain}:user`,
                  `edns:${net}:domain:${domain}:hosts`,
                  `edns:${net}:domain:${domain}:operators`
                );

                // Removed the domain from the address relationship
                const operators = await client.smembers(
                  `edns:${net}:domain:${domain}:operators`
                );
                if (operators.length) {
                  operators.forEach((operator) => {
                    batch = batch.srem(
                      `edns:${net}:account:${operator}:domain:operators`,
                      domain
                    );
                  });
                }
                ...
                const _hosts = await client.smembers(
                  `edns:${net}:domain:${domain}:hosts`
                );
                if (_hosts.length) {
                  dels.push(
                  ...
                  for (const _host of _hosts) {
                    async function exec(client: Redis): Promise<void> {
                      const _operators = await client.smembers(
                        `edns:${net}:host:${_host}.${domain}:operators`
                      );
                      if (_operators.length) {
                        _operators.forEach((operator) => {
                          batch = batch.srem(
                            `edns:${net}:account:${operator}:host:operators`,
                            `${_host}.${domain}`
                          );
                        });
                      }
            ...
            case EventType.SET_DOMAIN_OPERATOR: {
              ...
              if (data.approved) {
                await client
                  .pipeline()
                  .sadd(`edns:${net}:domain:${domain}:operators`, data.operator)
                  .sadd(
                    `edns:${net}:account:${data.operator}:domain:operators`,
                    domain
                  )
                  .exec();
              } else {
                await client
                  .pipeline()
                  .srem(`edns:${net}:domain:${domain}:operators`, data.operator)
                  .srem(
                    `edns:${net}:account:${data.operator}:domain:operators`,
                    domain
                  )
            ...
            case EventType.SET_DOMAIN_RESOLVER: {
            ...
            case EventType.SET_DOMAIN_USER: {
            ...
            case EventType.NEW_HOST: {
              ...
              const [user, expiry] = await client.hmget(
                `edns:${net}:domain:${domain}:user`,
                "user",
                "expiry"
              );
            ...
            case EventType.SET_HOST_OPERATOR: {
              ...
              if (data.approved) {
                await client
                  .pipeline()
                  .sadd(
                    `edns:${net}:host:${data.host}.${domain}:operators`,
                    data.operator
                  )
                  .sadd(
                    `edns:${net}:account:${data.operator}:domain:operators`,
                ...
                await client
                  .pipeline()
                  .srem(
                    `edns:${net}:domain:${data.host}.${domain}:operators`,
                    data.operator
                  )
                  .srem(
                    `edns:${net}:account:${data.operator}:host:operators`,
                    domain
                  )
                  .exec();
              }
            ...
            case EventType.SET_HOST_USER: {
              const data: ISetHostUserData = body.data;
              const domain = `${data.name}.${data.tld}`;

              await client.hmset(
                `edns:${net}:host:${data.host}.${domain}:user`,
                { user: data.newUser, expiry: data.expiry }
              );
            ...
            case EventType.SET_ADDRESS_RECORD: {
            ...
            case EventType.SET_MULTI_COIN_ADDRESS_RECORD: {
            ...
            case EventType.SET_TEXT_RECORD: {
            ...
            case EventType.BRIDGE_REQUESTED: {
              ...
              if (contracts?.addresses["Bridge"]) {
                ...
                if (req) {
                  ...
                  const [owner, expiry] = await client.hmget(
                    `edns:${net}:domain:${domain}:info`,
                    "owner",
                    "expiry"
                  );
                  ...
                  batch = batch.sadd(
                    `edns:${net}:account:${req.owner}:bridge:requested`,
                    data.ref
                  );
            ...
            case EventType.BRIDGE_ACCEPTED: {
              ...
              if (contracts?.addresses["Bridge"]) {
                ...
                if (req) {
                  ...
                  batch = batch.sadd(
                    `edns:${net}:account:${req.owner}:bridge:accepted`,
                    data.ref
                  );
                  await batch.exec();
                }
