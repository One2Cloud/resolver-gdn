...
export const putEvent = async (chainId: number, provider: DomainProvider, fqdn: string, type: EventType, data: any) => {
	...
	try {
		await sqs.sendMessage({
			...
			MessageBody: JSON.stringify({ type, data, chainId, mainnet: process.env.MAINNET == "1" }),
		});
	} catch (err) {
		console.error(err);
	}
