...
import { IRegistry } from 'src/contracts/ethereum/typechain/IRegistry';
import { PublicResolver } from 'src/contracts/ethereum/typechain/PublicResolver';
import { Registrar } from 'src/contracts/ethereum/typechain/Registrar';
import { IRegistry__factory } from 'src/contracts/ethereum/typechain/factories/IRegistry__factory';
import { PublicResolver__factory } from 'src/contracts/ethereum/typechain/factories/PublicResolver__factory';
import { Registrar__factory } from 'src/contracts/ethereum/typechain/factories/Registrar__factory';
...
export default class EdnsEthereumListener {
  ...
  private _listen_Registrar_DomainRegistered() {
    ...
    this.contracts.registrar.on(filter, async (name, tld, owner, expiry, event) => {
      const fqdn = `${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.DOMAIN_REGISTERED, {
        name,
        tld,
        owner,
        expiry,
  ...
  private _listen_Registrar_DomainRenewed() {
    ...
    this.contracts.registrar.on(filter, async (name, tld, expiry, event) => {
      const fqdn = `${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.DOMAIN_RENEWED, {
        name,
        tld,
        expiry,
      });
  ...
  private _listen_Registry_DomainBridged() {
    ...
    this.contracts.resolver.on(filter, async (name, tld, dstChain, event) => {
      const fqdn = `${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.DOMAIN_BRIDGED, {
        name,
        tld,
        dstChain,
      });
  ...
  private _listen_Registry_SetDomainOperator() {
    ...
    this.contracts.resolver.on(filter, async (name, tld, operator, approved, event) => {
      const fqdn = `${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.SET_DOMAIN_OPERATOR, {
        name,
        tld,
        operator,
        approved,
  ...
  private _listen_Registry_SetDomainUser() {
    ...
    this.contracts.resolver.on(filter, async (name, tld, newUser, expiry, event) => {
      const fqdn = `${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.SET_DOMAIN_USER, {
        name,
        tld,
        newUser,
        expiry,
  ...
  private _listen_Registry_NewHost() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, ttl, event) => {
      ...
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.NEW_HOST, {
        host,
        name,
        tld,
        ttl,
  ...
  private _listen_Registry_RemoveHost() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, event) => {
      const fqdn = `${host}.${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.REMOVE_HOST, {
        host,
        name,
        tld,
      });
  ...
  private _listen_Registry_SetHostOperator() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, operator, approved, event) => {
      const fqdn = `${host}.${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.SET_HOST_OPERATOR, {
        host,
        name,
        tld,
        operator,
  ...
  private _listen_Registry_SetHostUser() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, newUser, expiry, event) => {
      const fqdn = `${host}.${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.SET_HOST_USER, {
        host,
        name,
        tld,
        newUser,
  ...
  private _listen_Resolver_SetAddressRecord() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, address, event) => {
      const fqdn = `${host}.${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.SET_ADDRESS_RECORD, {
        host,
        name,
        tld,
        address,
  ...
  private _listen_Resolver_UnsetAddressRecord() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, event) => {
      const fqdn = `${host}.${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.UNSET_ADDRESS_RECORD, {
        host,
        name,
        tld,
      });
  ...
  private _listen_Resolver_SetMultiCoinAddressRecord() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, coin, address, event) => {
      const fqdn = `${host}.${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.SET_MULTI_COIN_ADDRESS_RECORD, {
        host,
        name,
        tld,
        coin,
  ...
  private _listen_Resolver_UnsetMultiCoinAddressRecord() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, coin, event) => {
      const fqdn = `${host}.${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.UNSET_MULTI_COIN_ADDRESS_RECORD, {
        host,
        name,
        tld,
        coin,
  ...
  private _listen_Resolver_SetTextRecord() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, text, event) => {
      const fqdn = `${host}.${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.SET_TEXT_RECORD, {
        host,
        name,
        tld,
        text,
  ...
  private _listen_Resolver_UnsetTextRecord() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, event) => {
      const fqdn = `${host}.${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.UNSET_TEXT_RECORD, {
        host,
        name,
        tld,
      });
  ...
  private _listen_Resolver_SetTypedTextRecord() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, type, text, event) => {
      const fqdn = `${host}.${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.SET_TYPED_TEXT_RECORD, {
        host,
        name,
        tld,
        type,
  ...
  private _listen_Resolver_UnsetTypedTextRecord() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, type, event) => {
      const fqdn = `${host}.${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.UNSET_TYPED_TEXT_RECORD, {
        host,
        name,
        tld,
        type,
  ...
  private _listen_Resolver_SetNFTRecord() {
    ...
    this.contracts.resolver.on(filter, async (host, name, tld, chainId, contractAddress, tokenId, event) => {
      const fqdn = `${host}.${name}.${tld}`;
      await putEvent(ServiceProvider.EDNS, fqdn, EventType.SET_NFT_RECORD, {
        host,
        name,
        tld,
        chainId,
