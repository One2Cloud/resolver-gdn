...
import { getNetworkConfig } from "../../../ethereum-network-config";
...
export const index: Handler<Input> = async (input) => {
  ...
  const contractAddresses = ContractAddresses.find(
    (contract) => contract.chainId === input.chainId
  );
  if (
    contractAddresses?.addresses["Registry.Diamond"] &&
    contractAddresses?.addresses["Registrar"] &&
    contractAddresses?.addresses["PublicResolver"]
  ) {
    const contracts = {
      ...
      registry: IRegistry__factory.connect(
        contractAddresses.addresses["Registry.Diamond"],
        provider
      ),
      bridge: contractAddresses.addresses["Bridge"]
        ? Bridge__factory.connect(
            contractAddresses.addresses["Bridge"],
            provider
          )
        : undefined,
    };
    ...
    switch (eventType) {
      case EdnsEventType.DOMAIN_REGISTERED: {
        ...
        const events = await contracts.registrar.queryFilter(
          filter,
          input.from,
          input.to
        );
      ...
      case EdnsEventType.SET_DOMAIN_OWNER: {
        ...
        const events = await contracts.registry.queryFilter(
          filter,
          input.from,
          input.to
        );
        ...
        for (const event of events) {
          ...
          const [_name, _tld] = await Promise.all([
      ...
      case EdnsEventType.SET_HOST_OPERATOR: {
        ...
        for (const event of events) {
          ...
          const [_host, _name, _tld] = await Promise.all([
            contracts.registry["getName(bytes32,bytes32,bytes32)"](
              host,
              name,
              tld
            ),
      ...
      case EdnsEventType.BRIDGE_REQUESTED: {
        if (contracts.bridge) {
          ...
          const events = await contracts.bridge.queryFilter(
            filter,
            input.from,
            input.to
          );
      ...
      case EdnsEventType.SET_ADDRESS_RECORD: {
        ...
        const events = await contracts.resolver.queryFilter(
          filter,
          input.from,
          input.to
        );
      ...
      case EdnsEventType.SET_NFT_RECORD: {
        ...
        synced = events.length;
        for (const event of events) {
          const { host, name, tld, chainId, contractAddress, tokenId } =
            event.args;
          const _host = ethers.utils.toUtf8String(host);
          const _name = ethers.utils.toUtf8String(name);
          const _tld = ethers.utils.toUtf8String(tld);
