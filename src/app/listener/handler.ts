...
const main = async (body: IBody): Promise<void> => {
  ...
  switch (body.eventType) {
    ...
    case EdnsEventType.SET_DOMAIN_OPERATOR: {
      ...
      if (data.approved) {
        ...
        await client
          ...
          .srem(`edns:${net}:account:${data.operator}:domain:operators`, domain)
          .exec();
      }
    ...
    case EdnsEventType.NEW_HOST: {
      ...
      await batch
        ...
        .set(`edns:${net}:host:${data.host}.${domain}:ttl`, data.ttl)
        .exec();
      break;
    }
    ...
    case EdnsEventType.REVALIDATE: {
      ...
      console.debug({ data });
      // Domain Register
      const currentOwner = await client.hget(
        `edns:${net}:domain:${domain}:info`,
        "owner"
      );
      if (!currentOwner) {
