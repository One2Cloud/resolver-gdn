...
export class EdnsV1FromContractService implements IEdnsResolverService {
  ...
  private _getContract(provider: ethers.providers.JsonRpcProvider): {
    ...
    if (provider.network.chainId === Network.IOTEX_TESTNET) {
      ...
      _REVERSE_REGISTRAR_ADDRESS_ =
        "0xCDEbE246529e6c5f549b08dC8DC9a720A34C3149";
    }
    if (provider.network.chainId === Network.IOTEX) {
      ...
      _REVERSE_REGISTRAR_ADDRESS_ =
        "0xaE2e725957f7EA2a5A5D7f73301ed5c3B23B06AA";
    }
    if (
      !_RESOLVER_ADDRESS_ ||
      !_REGISTRY_ADDRESS_ ||
    ...
    const registry = EDNSRegistry__factory.connect(
      _REGISTRY_ADDRESS_,
      provider
    );
    const reverse_registrar = ReverseRegistrar__factory.connect(
      _REVERSE_REGISTRAR_ADDRESS_,
      provider
    );
  ...
  public async getAddressRecord(
    ...
    if (!(await registry.recordExists(hash)))
  ...
  public async getMultiCoinAddressRecord(
    ...
    return {
      coin: input.coin,
      address: await resolver["addr(bytes32,uint256)"](
        hash,
  ...
  public async getNftRecord(
    ...
    if (!(await registry.recordExists(hash)))
      throw new DomainNotFoundError(input.fqdn);
    const [_chainId, _contractAddress, _tokenId] = await resolver.getNFT(
      hash,
      input.chainId
    );
    return {
