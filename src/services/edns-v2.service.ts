...
} from "../contracts/ethereum/edns-v2/typechain";
import { IOptions } from "../interfaces/IOptions.interface";
import {
  IEdnsRegistryService,
  IGetDomainOutput,
  IGetHostOutput,
} from "../interfaces/IEdnsRegistryService.interface";
...
const getContracts = (
  chainId: number
): { Registrar: Registrar; Registry: IRegistry; Resolver: PublicResolver } => {
  const NetworkConfig = getNetworkConfig();
  const network = NetworkConfig[chainId];
  const contracts = ContractAddress.find(
    (contract) => contract.chainId === network.chainId
  );
  if (
    contracts?.addresses["Registrar"] &&
    contracts?.addresses["Registry.Diamond"] &&
    contracts?.addresses["PublicResolver"]
  ) {
    try {
      ...
      const ResolverContract = PublicResolver__factory.connect(
        contracts.addresses["PublicResolver"],
        provider
      );
      const RegistryContract = IRegistry__factory.connect(
        contracts.addresses["Registry.Diamond"],
        provider
      );
      return {
  ...
  implements IEdnsResolverService, IEdnsRegistryService
{
  public async getBridgedEvent(
    ...
    if (name && tld) {
      owner = await redis.hget(
        `edns:${options?.net || Net.MAINNET}:domain:${name}.${tld}:info`,
        "owner"
      );
      content = await redis.get(
        `edns:${options?.net || Net.MAINNET}:account:${owner}:bridge:requested`
      );
    }
  ...
  public async getReverseAddressRecord(
    ...
    const fqdn = await redis.get(
      `edns:${options?.net || Net.MAINNET}:account:${
        input.address
      }:reverse_domain`
    );
    ...
    if (host && name && tld) {
      address =
        (await redis.hget(
          ...
          }:host:${host}.${name}.${tld}:records`,
          "address"
        )) || undefined;
    } else if (name && tld) {
      address =
        (await redis.hget(
          `edns:${options?.net || Net.MAINNET}:host:@.${name}.${tld}:records`,
          "address"
        )) || undefined;
    }
  ...
  public async getMultiCoinAddressRecord(
  ...
  public async getTypedTextRecord(
  ...
  public async getDomain(
    ...
    const results = await redis
      .pipeline()
      .hget(
        `edns:${options?.net || Net.MAINNET}:domain:${_domain}:info`,
        "owner"
      )
      .hget(
        `edns:${options?.net || Net.MAINNET}:domain:${_domain}:info`,
      ...
      .hget(
        `edns:${options?.net || Net.MAINNET}:domain:${_domain}:user`,
        "user"
      )
      .hget(
        `edns:${options?.net || Net.MAINNET}:domain:${_domain}:user`,
      ...
      .smembers(
        `edns:${options?.net || Net.MAINNET}:domain:${_domain}:operators`
      )
  ...
  public async getDomainsByAccount(
    account: string,
    options?: IOptions
  ): Promise<IGetDomainOutput[]> {
    const redis = createRedisClient();
    const _domains_ = await redis.smembers(
      `edns:${options?.net || Net.MAINNET}:account:${account}:domains`
    );
    const domains = await Promise.all(
      _domains_.map((d) => this.getDomain(d, options))
    );
  ...
  public async getHost(
    ...
    const results = await redis
      ...
      .smembers(
        `edns:${options?.net || Net.MAINNET}:host:${fqdn_}:records:list`
      )
      .exec();
  ...
  public async getTtl(
    ...
    const ttl = await redis.get(
      `edns:${options?.net || Net.MAINNET}:host:${fqdn}:ttl`
    );
    return ttl ? parseInt(ttl) : undefined;
  }

  public async getMultiCoinAddressList(
    fqdn: string,
    options?: IOptions
  ): Promise<IGetMultiCoinAddressListOutput> {
    const redis = createRedisClient();
    if (!isValidFqdn(fqdn)) throw new InvalidFqdnError(fqdn);
    if (!(await this.isExists(fqdn, options))) [];
    ...
    const { host, name, tld } = extractFqdn(fqdn);
    if (!host) fqdn = `@.${name}.${tld}`;
    const _list_ = await redis.smembers(
      `edns:${options?.net || Net.MAINNET}:host:${fqdn}:records:list`
    );
    const list = _list_
      .filter((r) => r.startsWith("multi_coin_address:"))
      .map((r) => r.replace("multi_coin_address:", ""));
    return { records_list: list };
  }

  public async getTypedTextList(
    fqdn: string,
    options?: IOptions
  ): Promise<IGetTypedTextListOutput> {
    const redis = createRedisClient();
    if (!isValidFqdn(fqdn)) throw new InvalidFqdnError(fqdn);
    if (!(await this.isExists(fqdn, options))) [];
    if (await this.isExpired(fqdn, options)) throw new DomainExpiredError(fqdn);
    ...
    const list = _list_
      .filter((r) => r.startsWith("typed_text:"))
      .map((r) => r.replace("typed_text:", ""));
    return { records_list: list };
  }
}

export class EdnsV2FromContractService
  implements IEdnsResolverService, IEdnsRegistryService
{
  private async _getDomainChainId(
    domain: string,
    options?: IOptions
  ): Promise<number> {
    const redis = createRedisClient();
    const { host, name, tld } = extractFqdn(domain);
    const inContractChain = await redis.hget(
      `edns:${options?.net || Net.MAINNET}:domain:${name}.${tld}:info`,
      "chain"
    );
  ...
  public async getAddressRecord(
    ...
    if (host && name && tld) {
      result = {
        address: await contracts.Resolver.getAddress(
  ...
  public async getTextRecord(
    ...
    if (host && name && tld) {
      result = {
        text: await contracts.Resolver.getText(
          ethers.utils.toUtf8Bytes(host),
          ethers.utils.toUtf8Bytes(name),
          ethers.utils.toUtf8Bytes(tld)
        ),
  ...
  public async getTypedTextRecord(
    ...
    if (host && name && tld) {
      result = {
        typed: input.typed,
        text: await contracts.Resolver.getTypedText(
          ethers.utils.toUtf8Bytes(host),
          ethers.utils.toUtf8Bytes(name),
          ethers.utils.toUtf8Bytes(tld),
          _typed
  ...
  public async isExists(
    ...
    if (host && name && tld) {
      ...
      return await contracts.Registry["isExists(bytes32,bytes32)"](
        ethers.utils.solidityKeccak256(["string"], [name]),
        ethers.utils.solidityKeccak256(["string"], [tld])
      );
    } else {
      return await contracts.Registry["isExists(bytes32)"](
        ethers.utils.keccak256(tld)
      );
    }
  }
  ...
  public async isExpired(
    ...
    const { host, name, tld } = extractFqdn(fqdn);
    if (name && tld) {
      time = await contracts.Registry["getExpiry(bytes32,bytes32)"](
        ethers.utils.solidityKeccak256(["string"], [name]),
        ethers.utils.solidityKeccak256(["string"], [tld])
      );
    } else {
      time = await contracts.Registry["getExpiry(bytes32)"](
        ethers.utils.keccak256(tld)
      );
    }
  ...
  public async getTtl(
    ...
    if (!isValidFqdn(fqdn)) throw new InvalidFqdnError(fqdn);
    const _chainId =
      options?.chainId || (await this._getDomainChainId(fqdn, options));
    if (!(await this.isExists(fqdn, options, _chainId)))
      throw new DomainNotFoundError(fqdn);
    if (await this.isExpired(fqdn, options, _chainId))
      throw new DomainExpiredError(fqdn);
    const contracts = getContracts(_chainId);
    const { host, name, tld } = extractFqdn(fqdn);
    if (host && name && tld) {
  ...
  public async getOwner(
    ...
    const { host, name, tld } = extractFqdn(fqdn);
    if (name && tld) {
      const owner = await contracts.Registry["getOwner(bytes32,bytes32)"](
        ethers.utils.solidityKeccak256(["string"], [name]),
        ethers.utils.solidityKeccak256(["string"], [tld])
      );
      return owner;
  ...
  public async getExpiry(
    ...
    const { host, name, tld } = extractFqdn(fqdn);
    if (name && tld) {
      const expiry = await contracts.Registry["getExpiry(bytes32,bytes32)"](
        ethers.utils.solidityKeccak256(["string"], [name]),
        ethers.utils.solidityKeccak256(["string"], [tld])
      );
