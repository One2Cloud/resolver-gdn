...
import {
  EdnsV2FromContractService,
  EdnsV2FromRedisService,
} from "./edns-v2.service";
...
export class EdnsService implements IEdnsResolverService {
  ...
  public async getReverseAddressRecord(
    ...
    if (!output && !options?.onchain) {
      // Get the reverse address from Redis by default
      output = await this._v2RedisService.getReverseAddressRecord(
        input,
        options
      );
    }
    if (!output) {
      ...
      output = await this._v2ContractService.getReverseAddressRecord(
        input,
        options
      );
      cache = "miss";
  ...
  public async getAddressRecord(
    ...
    if (output && cache === "miss") {
      ...
      if (options?.chainId) {
        const isExists = await this._v2ContractService.isExists(
          input.fqdn,
          options
        );
        if (isExists) {
          await putSqsMessage({
            ...
            net: EdnsMainnets.includes(options.chainId)
              ? Net.MAINNET
              : Net.TESTNET,
          });
        }
  ...
  public async getMultiCoinAddressRecord(
    ...
    if (!output && !options?.onchain) {
      output = await this._v2RedisService.getMultiCoinAddressRecord(
        input,
        options
      );
    }
    if (!output) {
      output = await this._v2ContractService.getMultiCoinAddressRecord(
        input,
        options
      );
      cache = "miss";
  ...
  public async getMultiCoinAddressList(
    fqdn: string,
    options?: IOptions
  ): Promise<IGetMultiCoinAddressListOutput | undefined> {
    ...
    if (!output && !options?.onchain) {
      output = await this._v2RedisService.getMultiCoinAddressList(
        fqdn,
        options
      );
    }
  ...
  public async getTypedTextList(
    fqdn: string,
    options?: IOptions
  ): Promise<IGetTypedTextListOutput | undefined> {
