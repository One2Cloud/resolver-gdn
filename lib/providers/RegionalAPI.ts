...
export class RegionalAPI extends Construct {
	...
	constructor(scope: Construct, id: string, props: IConstructProps) {
		...
		this.service = new apprunner.Service(this, "Service", {
			serviceName: "resolver-gdn-query-api",
			source: apprunner.Source.fromEcr({
				...
				imageConfiguration: {
					port: 8080,
					environmentSecrets: {
						REDIS_URL: apprunner.Secret.fromSecretsManager(props.secret, "REDIS_URL"),
						GETBLOCK_CONFIG: apprunner.Secret.fromSecretsManager(props.secret, "GETBLOCK_CONFIG"),
						GROVE_CITY_APP_ID: apprunner.Secret.fromSecretsManager(props.secret, "GROVE_CITY_APP_ID"),
						SUBGRAPH_URL: apprunner.Secret.fromSecretsManager(props.secret, "SUBGRAPH_URL"),
					},
		...
		props.secret.grantRead(this.service);
